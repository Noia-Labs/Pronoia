...
#
# 用法:
#   scripts/start_local.sh            # 启动（已在运行则跳过）
#   scripts/start_local.sh stop       # 停止（按端口找进程组）
#   scripts/start_local.sh restart    # 重启
#   scripts/start_local.sh status     # 查看运行状态
#   scripts/start_local.sh logs backend|frontend   # 跟踪日志
set -euo pipefail
